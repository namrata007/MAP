...
remove=[] #stores a list of noisy columns based on third criteria
keep=[] #stores a list of non noisy columns based on third criteria
...
#If the don't have the same length, issue warning
 ...
 #check if the column is noisy based on second criteria
        ...
        if(total>((len(seqs))/2)): #checks if more than fifty percent of the column positions are gaps
            noisy= True
        else:
            noisy= False
