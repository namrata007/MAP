...
try:
	...
	num_indels= 0        
		...
		noisy12 = False		#whether the noise is due to condition 1 or 2
		noisy3 = True		#whether the noise is due to condition 3
                for seq in seqs:                 #go through all the sequences
			if seq[col] == '-':
				num_indels += 1
                        elif seq[col] in 'ARNDCEQGHILKMFPSTWYV':
                                if seq[col] in count:
                                        count[seq[col]] += 1
                                else:
                                        count[seq[col]] = 1
			if len(count)>=(check/2) or total>(check/2):
				noisy12 = True
				break
			for k in count:
